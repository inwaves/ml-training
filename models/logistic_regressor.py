...
def preprocess_data():
    ...
    X = diabetes_df.drop(
        ["outcome", "age", "skinthickness"], axis=1
...
def calc_model_perm_importance(X_test, y_test, lin_model):
    ...
    return eli5.sklearn.PermutationImportance(lin_model, random_state=1).fit(
        X_test, y_test
    )


def print_results(
    X_test, y_test, lin_model, predictions_lin_model, solver, confusion_matrix=False
):
...
def logistic_regression(X_train, X_test, y_train, y_test):
    ...
    for solver in ["newton-cg", "lbfgs", "liblinear", "sag", "saga"]:
        lin_model = parameterise_model(X_train, y_train, solver, C=0.8, max_iter=1000)
    ...
    show_partial_dep_plots(
        parameterise_model(X_train, y_train, solver="newton-cg"), X_test
    )
